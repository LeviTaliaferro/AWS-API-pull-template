...
CHUNK_SIZE = 10000
#modify based on API best practice and memory contraints

def get_num_records():
    #placeholder function to replicate GET
    #https://jsonplaceholder.typicode.com/number_of_users call
    Return 100000
...
def get_data(
    ...
    try:
        r =http_request(
            ...
            fields={"start_user_id": start_user_id: end_user_id},
            )
...
#convert json data into a row format string
def parse_data(json_data):
    return f'{json_data.get("userId")},{json_data["id"]},"{json_data["title"]}"\n'

#write to local, copy to S3 before end of session yeet
...
import urllib3
#this makes tmp not temp
s3_client = boto3.client("s3")
LOCAL_FILE_SYS = "/tmp"
S3_BUCKET = "your-s3-bucket"  # PLACEHOLDER, REPLACE ON EACH INSTANCE FFS
CHUNK_SIZE = 10000  #based on API, memory constraints, experimentation
